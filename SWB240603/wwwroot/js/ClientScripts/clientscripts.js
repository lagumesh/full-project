...
$(document).ready(function () {
    ...
    function calculateAge(dob) {
        ...
        if (selectedDate.length !== 3) {
            $('#ageof40').val("Select Valid Date");
            return;
        }

        var day = parseInt(selectedDate[0]);
        var month = parseInt(selectedDate[1]);
        var year = parseInt(selectedDate[2]);
        ...
        // Adjust age and month difference if necessary
        if (monthDiff < 0 || (monthDiff === 0 && dayDiff < 0)) {
            age--;
            monthDiff += 12;
        }

        // Adjust day difference if necessary
        if (dayDiff < 0) {
            monthDiff--;
        ...
        // Generate the age string
        var ageString = '';
        if (isNaN(age) || isNaN(monthDiff) || isNaN(dayDiff)) {
            ageString = "Select Valid Date";
        } else {
            ageString = age + ' years, ' + monthDiff + ' months and ' + dayDiff + ' days old.';
        }
...
$(document).ready(function () {
    ...
    $('input[name="Applicant.Reservation.IsClamingKalyanaKarnatakaReservation"]').change(function () {
        $('#kalyani input[type="radio"]:checked').prop('checked', false);
        var selectedValue = $(this).val();
...
$(document).ready(function () {
    ...
    // ID Card Preview
     ...
     $('#Applicant_Uploads_IdentityCard').change(IDImagePreview); 
        ...
        $('#IdUpload').click(function () {
            ...
            $('#Applicant_Uploads_IdentityCard').click();
        });


    // Signature Preview

    $('#Applicant_Uploads_Signature').change(SignImagePreview);
    ...
    $('#signUpload').click(function () {
        // Trigger the same logic as when the file input changes
        $('#Applicant_Uploads_Signature').click();
    });


    //Thumb Image Preview

    $('#Applicant_Uploads_Thumb').change(ThumbImagePreview);
    ...
    $('#thumbUpload').click(function () {
        // Trigger the same logic as when the file input changes
        $('#Applicant_Uploads_Thumb').click();
    });

    // Photo Image Preview
    $('#Applicant_Uploads_Photo').change(PhotoImagePreview);
...
$(document).ready(function () {
    ...
    $('#sslcgrade, #modalsslcgrade, #sslcgradeobtained, #modalsslcgradeobtained, #sslcautopercentage, #modalsslcautopercentage, #sslcmanualpercentage, #modalsslcmanualpercentage').hide();
...
$(document).ready(function () {
    ...
    $("input[name='Applicant.Reservation.IsClaimingExServicemenRelationReservation']").change(function () {
        if ($(this).val() === "Yes") {
            ...
            $(".group-2b input[type='radio']").prop("checked", false);
            $('.group-2e select').val("");
        }
    });
});
...
$(document).ready(function () {
    $('#exservicemenrelation').hide()
    $('#modalexservicemenrelation').hide()
    $('#directorexservice').hide()
    $('#modaldirectorexservice').hide()
    ...
    $('#Applicant_ApplyingTypeCode').change(function () {
        ...
        $('.group-2').hide()
        $('.group-2b').hide()
        $('#exservicemenrelation select').val('')
        $('#exservicemenrelation input[type="radio"]').prop('checked', false);
        $('#directorexservice input[type="radio"]').prop('checked', false);
        var sec = $(this).val();
        // alert(sec)
        ...
        if (sec === 'DIR') {
            // alert('hh')
            $('#exservicemenrelation  input[type="date"] select').val('')
            $('#exservicemenrelation').show()
            $('#modalexservicemenrelation').show()
            $('#directorexservice').hide()
            $('#modaldirectorexservice').hide()
            $('#exservicemensection').hide();
            $('#modalexservicemensection').hide();
            ...
            $('#exservicemenrelation').hide()
            $('#modalexservicemenrelation').hide()
            $('#directorexservice').show()
            $('#modaldirectorexservice').show()
...
$(document).ready(function () {
    $('#Applicant_AadharNo').change(function () {
        ...
        var cardType = $('#Applicant_Uploads_CardType').val();
        if (cardType === 'AD') {
            $('#Applicant_UploadedIDNo').val(aadharNumber);
        }
    });

    $('#Applicant_Uploads_CardType').change(function () {
